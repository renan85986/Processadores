...
typedef struct {
    ...
    XNmp_axi4l_ip_uio_map maps[ MAX_UIO_MAPS ];
} XNmp_axi4l_ip_uio_info;

/***************** Variable Definitions **************************************/
static XNmp_axi4l_ip_uio_info uio_info;
...
static int uio_info_read_name(XNmp_axi4l_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XNmp_axi4l_ip_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XNmp_axi4l_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XNmp_axi4l_ip_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XNmp_axi4l_ip_Initialize(XNmp_axi4l_ip *InstancePtr, const char* InstanceName) {
	XNmp_axi4l_ip_uio_info *InfoPtr = &uio_info;
